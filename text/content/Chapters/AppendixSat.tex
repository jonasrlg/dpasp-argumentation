...
% \textit{proof theory} resides on the next chapter about \ac{ASP}
% [\ref{ch:pasp}], in which the need for efficient algorithms to
% solve \textit{satistiability} problems is crucial for the
% development of \ac{ASP} solvers. Moreover, this intrinsic
...
% such as \acp{BDD} \citep{akers1978binary, bryant1986graph}, that
...
% as \ac{SAT} and \textit{model counting}
% \citep{akers1978binary, bryant1986graph}.

% This relation between \ac{SAT} and \ac{CT}, despite being a
...
% complexity of developing efficient methods for solving \ac{PASP}
% and other \ac{PLP} problems; what makes the development of
...
\ac{SAT}-like problems, we only introduce basic logic concepts, such as
\textit{Boolean formulas}, \acp{CNF} and \textit{Logical Propositional
Theories}.
...
\begin{definition}[Disjunctive Normal Form]
    A Boolean formula over variables $u_1, \ldots u_n$ is in
    \ac{DNF} if it is a disjunction of clauses, where each
    clause is a conjunction of literals. Hence, a \ac{DNF} is a
...
The \ac{SAT} problem is a fundamental problem in both \ac{AI} and
\ac{ToC}. This importance is due
not only by the logical nature of \ac{SAT}, but also by the fact
that all \ac{NP} problems can be reduced to \ac{SAT} in
polynomial time, thus its \ac{NP}-completeness \citep{Cook_1971,
levin1973universal}. In this manner, the \ac{SAT} problem is a
natural candidate for encoding not only arbitrary a wide range
of logical theories, but also for encoding any \ac{NP} problem
in general. Therefore, methods to solve \ac{SAT} fast are of
great interest.

Later, in chapters \ref{ch:pasp} and \ref{ch:compilation}, when
we discuss \acl{ASP} and \acl{KC}, we will see that being able
to solve numerous logical queries in polynomial time is a
desirable feature for many logical applications, such as
\aclp{PLP}, the focus of this dissertation. Therefore, our goal
will not be to solve \ac{SAT} only once; the necessity of
solving it many times creates the need to develop more scalable
methods for solving multiple queries.

Thus, we begin this thesis with a formal definition of the
\acl{SAT} problem and its variations, such as \ac{MAX-SAT} and
\ac{Sharp-SAT} (also known as \ac{SAT}-Counting).

\subsection{SAT}

The Boolean Satisfatiability Problem is the problem of
determining whether a given \ac{CNF} is \textit{satisfiable},
...
\begin{definition}[SAT]
    Let \ac{SAT} be the language of all satisfiable \acp{CNF}.
    Then, we say that for a \ac{CNF} $\phi$, the \ac{SAT}
...
satisfiable. On the other hand, the \ac{CNF} $x \land
\overline{x}$ is unsatisfiable, since it is a
\textit{contradiction} (the opposite of a tautology).

\subsection{MAX-SAT}

The \ac{MAX-SAT} problem is the problem of finding an assignment
that satisfies the maximum number of clauses in a given \ac{CNF}
\citep{KRENTEL1988490}.

\begin{definition}[MAX-SAT]  Formally, let $\phi$ be a
    \ac{CNF} over variables $x_1, \ldots, x_n$. Then, the
    \ac{MAX-SAT} problem is to find an assignment $a$ such that
    \begin{argmaxi*}|l|
        {a}{ \sum_{c \in C} [\!\![ c(a) ]\!\!] }
        {}{}
...
Sometimes, the \ac{MAX-SAT} problem refers to the weighted
...
\begin{definition}[Weighted MAX-SAT]
    Formally, let $\phi$ be a \ac{CNF} over variables $x_1,
    \ldots, x_n$, and let $w: C \rightarrow \mathbb{R}^+$ be the
    weight function over the clauses of $\phi$. Then, this
    variant of the \ac{MAX-SAT} problem, called
    \textit{Weighted} \ac{MAX-SAT}, is defined as the problem of
...
An interesting difference between the \ac{SAT} and \ac{MAX-SAT}
...
\ac{SAT} is a special case of the \ac{MAX-SAT}, where it is
possible to obtain a solution where the maximum number of
possible clauses satisfied is the total number of clauses. Thus,
there is a reduction from \ac{SAT} to \ac{MAX-SAT}, which is
polynomial in time. Moreover, because of this reduction, one can
claim that the \ac{MAX-SAT} problem is \ac{NP}-hard, since
\ac{SAT} is \ac{NP}-complete.

The reduction from \ac{MAX-SAT} to weighted \ac{MAX-SAT} is
simple too: just set all weights to $1$ (or any other constant),
and the problem is reduced to the unweighted version. Therefore,
the weighted version of the problem is also \ac{NP}-hard, as one
would expect. Moreover, the \ac{MAX-SAT} problem is also
\ac{OptP} complete, similar to the \textit{Traveling Salesman}
and \textit{Knapsack} problems \citep{KRENTEL1988490}.

\subsection{Sharp-SAT}

Differently from the \ac{SAT} and \ac{MAX-SAT} problems, the
$\#SAT$ (or \ac{Sharp-SAT}) problem is a counting problem, where
the goal is to count the number of solutions of a given \ac{CNF}
has \citep{valiant1979189}. Therefore, one can see the
\ac{Sharp-SAT} problem closer to Levin studied problems
...
variables of the \ac{CNF}, and count the number of them that
...
This relation between \ac{Sharp-SAT} and \ac{SAT} variations
does not stop here. The \ac{Sharp-SAT} is clearly \ac{NP}-hard,
since it is possible to reduce \ac{SAT} to \ac{Sharp-SAT} in
polynomial time, because knowing the number of solutions of a
\ac{CNF} is equivalent to knowing whether it has at least one
solution. Moreover, \ac{Sharp-SAT} is a \ac{Sharp-P}-complete
problem (or $\#P\mathrm{-complete}$), which means that it is
capable of reducing the set of the counting problems associated
with the decision problems in the set \ac{NP}.

\begin{definition}[\#SAT]
    Formally, let $\phi$ be a \ac{CNF} over variables $x_1,
    \ldots, x_n$. Then, the \ac{Sharp-SAT}  problem is to find
