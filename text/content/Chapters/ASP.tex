...
%
Now, we introduce the concept of programs that include negation in their
...
To reason about the complexity of logic programs we need additional vocabulary.
In particular, the notions of \textit{Herbrand Universe} and \textit{Herbrand
Base} allow us to characterise ground instances while balancing expressiveness
and efficiency:
...
\begin{definition}[Herbrand Universe]
    ...
    and functions in $P$ (with respect to a predefined vocabulary $L$).
    The \textit{Herbrand Base} $HB(P)$ of $P$ is the set
    of all ground atoms that can be formed from terms in $HU(P)$ and
    predicates of $P$. Finally, a \textit{Herbrand
    interpretation} is any subset $I \subseteq HB(P)$ that denotes the ground
    atoms considered true.
\end{definition}
...
With the notion of Herbrand Universe and Base, we can now give
a more formal definition of \textit{grounding}
\citep{eiter2009answer}:

\begin{definition}[Grounding]
    A ground instance of a clause $C$ in a logic
    program $P$ is any clause $C'$ obtained from $C$ by
    applying a substitution
    \[
        \theta: \mathrm{Var}(C) \rightarrow HU(P),
    \]
    where $\mathrm{Var}(C)$ denotes the variables
    occurring in $C$. The grounding of $P$ is the set
    of all ground instances of its clauses and
    is denoted by $\mathrm{ground}(P) = \bigcup_{C \in P} \mathrm{ground}(C)$.
\end{definition}
...
\begin{definition}[Interpretation]
   ...
   \begin{itemize}
        \item A ground clause $C =$ \mintinline{prolog}{a :- b1, ..., bM, not c1, ..., not cN},
   ...
   I$. Normal logic programs can have multiple minimal models,
   whereas definite logic programs admit a unique minimal model
   \citep{eiter2009answer}.
\end{definition}
...
In the context of this work, we will call stable models
\textbf{\textit{answer sets}}. Usually, we call the stable models
of a program $P$ \textit{answer sets} when we are talking about
Extended Logic Programs (ELP), which can contain \textit{strongly
negated} atoms, but we do not define these ELPs, and instead
recommend the reader to refer to \citep{eiter2009answer}.
...
\begin{definition}[Interpretation (of an EDLP)]
    ...
    of:
    \begin{enumerate}
        \item A ground clause $C =$ \mintinline{prolog}{a1; ...; aK :- b1, ..., bM, not c1, ..., not cN},
...
\begin{definition}[Probabilistic Fact]
    ...
    ungrounded. When considering this case, we interpret such a
    probabilistic fact as the set of all grounded probabilistic
    facts, obtained after grounding the variables in the atom.

    Finally, probabilistic facts do not unify with the
    head of any rule: there is no substitution $\theta$
    that makes a probabilistic fact $f$ and a rule head $h$
    syntactically identical, i.e.\ $f\theta \ne h\theta$ for
    all choices of $f$ and $h$.
\end{definition}
...
probabilistic facts to the head of rules.
...
\begin{definition}[Annotated Disjunctive Rules]
    ...
    \begin{minted}{prolog}
        p::a
    \end{minted}
    is a special case (and syntactic sugar) of an ADR, where
    \mintinline{prolog}{p::a} is equivalent to the rule
    \mintinline{prolog}{p::a; (1-p)::dummy :- .}, with a fresh atom
    \mintinline{prolog}{dummy} that is never derived
    \citep{geh2023dpasp}.
\end{definition}
...
a more formal description is required here, together with adaptations that make
the semantics compatible with ASP programs under stable models.

It is important to emphasise that probabilities are assigned to entire PLPs,
not to individual logical consequences. This makes
probabilistic semantics over PLPs agnostic with respect to the
underlying logical semantics, which is particularly useful
...
we can conclude by earlier subsections, particularly
