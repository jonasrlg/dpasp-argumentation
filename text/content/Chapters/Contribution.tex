...
before grounding (Listing~\ref{lst:ungrounded_pasp_1} and Listing~\ref{lst:ungrounded_pasp_2}):

\begin{listing}[ht]
\begin{minted}[fontsize=\small,breaklines]{prolog}
% Annotated disjunctions defining component types, where component(ARG,TYPE)
%   TYPE = 0: Premise
%   TYPE = 1: Claim
%   TYPE = 2: Major Claim
0.5::component(ARG,0); 0.25::component(ARG,1); 0.25::component(ARG,2) :- node(ARG).
...
%
% Major Claims are never the source of relations
fail :- node(SRC), node(DST), SRC != DST, component(SRC,2), not relation(SRC,DST).
% Claims can only relate to Major Claims
fail :- node(SRC), node(DST), SRC != DST, component(SRC,1), not component(DST,2), not relation(SRC,DST).
% Premises cannot be the source of Major Claims
fail :- node(SRC), node(DST), SRC != DST, component(SRC,0), component(DST,2), not relation(SRC,DST).
\end{minted}
\caption{Stratified (simplified) PASP program for argumentation constraints.}
\label{lst:ungrounded_pasp_1}
\end{listing}

\begin{listing}[ht]
\begin{minted}[fontsize=\small,breaklines]{prolog}
% 1. Component Assignment (Annotated Disjunction)
% component(ARG, TYPE):
%   TYPE = 0: None (No component selected)
%   TYPE = 1: Premise
%   TYPE = 2: Claim
%   TYPE = 3: Major Claim
0.25::component(X, 0); 0.25::component(X, 1); 0.15::component(X, 2); 0.75::component(X, 3) :- node(X).

% 2. Relation Assignment (Annotated Disjunction)
% relation(SRC, DST, TYPE):
%   TYPE = 0: None (No relation)
%   TYPE = 1: Support Attack (Used for 'Support' logic)
%   TYPE = 2: Attack (Used for 'Attack' logic)
0.33::relation(SRC, DST, 0); 0.33::relation(SRC, DST, 1); 0.33::relation(SRC, DST, 2) :- node(SRC), node(DST), SRC != DST.

% --- Structural Constraints (Fail-Based Pruning) ---
% These rules prune models that violate the desired graph structure based on component types.
% We use 'not relation(SRC,DST,0)' as a shorthand for 'a relation exists' between SRC and DST.

% Structural Rule 1: Claims cannot relate to other Claims.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 2), component(DST, 2), not relation(SRC, DST, 0).
% Structural Rule 2: Major Claims cannot be the source of relations to Claims.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 3), component(DST, 2), not relation(SRC, DST, 0).
% Structural Rule 3: Claims cannot be the source of relations to Premises.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 2), component(DST, 1), not relation(SRC, DST, 0).
% Structural Rule 4: Major Claims cannot be the source of relations to Premises.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 3), component(DST, 1), not relation(SRC, DST, 0).

% --- Acceptability and MDQ Machinery (Generalized Rules) ---

% hasRelation(X, Y) is true if a non-zero relation exists and the structural constraints are satisfied (implicitly by 'fail' rules).
hasRelation(X, Y) :- relation(X, Y, 1).
hasRelation(X, Y) :- relation(X, Y, 2).

% Incoming relations act as a first notion of acceptability
hasIncomingRelation(X) :- hasRelation(Y, X).
% Acceptability is determined by the absence of incoming relations
acceptable(X) :- node(X), not hasIncomingRelation(X).

% Intermediate steps for acceptability
% A support is "not_supports" if the supporter is NOT acceptable.
not_supports(X, Y) :- node(X), node(Y), relation(X, Y, 1), hasRelation(X, Y), not acceptable(X).
% An attack is "effective" if the attacker IS acceptable.
effectiveAttacks(X, Y) :- node(X), node(Y), relation(X, Y, 2), hasRelation(X, Y), acceptable(X).

% hasSupport: there exists an acceptable supporter Y that supports X
% Note: Using not/1 for existence check.
% absenceSupport(X) is true if there is ANY supporter Y whose support is NOT effective (i.e., not_supports(Y,X) is true).
absenceSupport(X) :- not_supports(Y, X), node(Y), node(X).
hasSupport(X) :- node(X), relation(Y, X, 1), not absenceSupport(X).

% acceptable(X) is true if it has support (from acceptable arguments) AND is not attacked by an acceptable argument.
attackedByAcceptable(X) :- effectiveAttacks(Y, X).
acceptable(X) :- node(X), hasSupport(X), not attackedByAcceptable(X).

% Initial Acceptability (Base case for recursion, if needed/desired)
% This part is often necessary in simpler frameworks, but the recursive definition handles it if the graph is properly built.
% acceptable(X) :- node(X), not hasIncoming(X).

% MDQ (Maximum Degree Quality)
% hasMC: there is at least one Major Claim (component(...,3))
hasMC :- component(X, 3), node(X).
% not_justified(X): A Major Claim is not acceptable
not_justified(X) :- node(X), component(X, 3), not acceptable(X).
% exists_not_justified: There is at least one Major Claim that is not acceptable
exists_not_justified :- not_justified(X).
% mdq: There is a Major Claim AND no Major Claim is unjustified
mdq :- hasMC, not exists_not_justified.
\end{minted}
\caption{Non-stratified PASP program for argumentation constraints.}
\label{lst:ungrounded_pasp_2}
\end{listing}

\paragraph{Grounding.} The grounding step instantiates the above program over a
set of arguments provided as input. For an input size of $n$ arguments over
Listing~\ref{lst:ungrounded_pasp_1}, the grounded program contains $3n$
probabilistic facts for the components and $n(n-1)/2$ probabilistic facts for
the relations, resulting in a total of $3n + n(n-1)/2$ probabilistic facts. Note
that the number of rules in the grounded program grows quadratically with $n$
due to the pairwise relations between arguments. In order to compute such
groundings, we use \emph{clingo}, the state-of-the-art ASP grounder
\citep{gebser2014clingo}.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{content/gfx/argumentation_stratified_scatter.pdf}
        \caption{Scatter plot comparing circuit size (number of nodes plus edges) for top-down
        compilers over Listing~\ref{lst:ungrounded_pasp_1}.}
        \label{fig:scatter_circuit_size}
    \end{subfigure}%
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{content/gfx/argumentation_stratified_time.pdf}
        \caption{Scatter plot comparing compilation time (in seconds) for top-down compilers
        over Listing~\ref{lst:ungrounded_pasp_1}.}
        \label{fig:scatter_compilation_time}
    \end{subfigure}
    \caption{Comparison of top-down compilers over Listing~\ref{lst:ungrounded_pasp_1}.
    The left subfigure shows circuit size (number of nodes plus edges), while the right
    subfigure shows compilation time (in seconds).  Larger dots represent larger values
    on the y-axis.}
    \label{fig:scatter_comparison}
\end{figure}

The experiments in Figure~\ref{fig:scatter_comparison} demonstrate that, for the PASP
program in Listing~\ref{lst:ungrounded_pasp_1}, the \textsc{D4} compiler outperformed
the other compilers in both compilation time and circuit size.
This makes D4 a compelling choice for the stratified argumentation mining
instances considered in this work. The decision to limit the evaluation
to argumentation graphs with up to $50$ nodes stems from the fact that the
datasets currently of interest to our research group contain graphs with at
most half this size.

It is worth noting, however, that while \textsc{D4} was the most efficient in
this setting, the other compilers also produced compact circuits. This is
particularly relevant in the context of Neuro-Symbolic frameworks, where the
final circuit sizes remain manageable and can be encoded efficiently on a GPU.
This property ensures that the compiled circuits are suitable for integration
into end-to-end learning pipelines, even when using compilers other than
\textsc{D4}.
...
\begin{table}[ht]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tabular}{llll}
            \toprule
            \# Nodes & \textsc{C2D} & \textsc{D4} & \textsc{SharpSAT-TD} \\
            program &  &  &  \\
            \midrule
            1 & 107 & 99 & 160 \\
            2 & 2555 & 2736 & 1637 \\
            3 & 170844 & 123025 & 88729 \\
            4 & 101566925 & 6183424 & 2057170 \\
            5 & - & - & 21386450 \\
            6 & - & - & 436225040 \\
            \bottomrule
        \end{tabular}
        \caption*{Circuit Size (Number of Nodes + Edges)}
        \label{tab:circuit_size}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \begin{tabular}{llll}
            ...
            program &  &  &  \\
            \midrule
            1 & 0.001 & 0.00014 & 0.001225 \\
            2 & 0.002 & 0.00105 & 0.003164 \\
            3 & 0.044 & 0.04816 & 0.016818 \\
            4 & 37.9 & 3.96765 & 0.264196 \\
            5 & - & - & 2.93743 \\
            6 & - & - & 65.4520 \\
            \bottomrule
        \end{tabular}
    ...
    \end{minipage}
    \caption{Comparison of top-down compilers over Listing \ref{lst:ungrounded_pasp_2}.
    On the left, we have a table comparing circuit size (number of nodes plus edges);
    on the right, compilation time (in seconds).}
    \label{tab:comparison}
\end{table}

On the other hand, the non-stratified results, displayed in Table~\ref{tab:comparison},
highlight the challenges associated with compiling non-stratified PASP programs, such
as the one in Listing~\ref{lst:ungrounded_pasp_2}, which is of primary interest to
our work. Unlike the stratified program in Listing~\ref{lst:ungrounded_pasp_1}, the
non-stratified program introduces significantly more complex probabilistic-logical
knowledge and constraints, leading to exponential blow-ups in both circuit size and
compilation time, even for small instance sizes, which can be easily observed in
Tables~\ref{tab:circuit_size} and~\ref{tab:compilation_time}.

For instance, while stratified programs remain manageable across all compilers, the
non-stratified program quickly becomes infeasible for \textsc{C2D} and \textsc{D4} as
the number of nodes increases. In contrast, \textsc{SharpSAT-TD} demonstrates superior
scalability, producing smaller circuits and completing compilation in significantly
less time compared to the other compilers. This makes \textsc{SharpSAT-TD} the most
promising option for handling the intricate requirements of non-stratified PASP modeling.
