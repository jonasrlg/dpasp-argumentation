...
\begin{listing}[ht]
\begin{minted}[fontsize=\small,breaklines]{prolog}
% 1. Component Estimation, where TYPE in component(X, TYPE) denotes:
%   0: None / 1: Premise / 2: Claim / 3: Major Claim
0.25::component(X, 0); 0.25::component(X, 1); 0.15::component(X, 2); 0.35::component(X, 3) :- node(X).
% 2. Relationship Prediction, where TYPE in relation(SRC, DST, TYPE) denotes:
...
% Structural Rule 2: Major Claims cannot be the source to Claims.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 3), component(DST, 2), not relation(SRC, DST, 0).
% Structural Rule 3: Claims cannot be the source to Premises.
fail :- node(SRC), node(DST), SRC != DST, component(SRC, 2), component(DST, 1), not relation(SRC, DST, 0).
% Structural Rule 4: Major Claims cannot be the source to Premises.
...
% absenceSupport is true if there is ANY supporter whose support is NOT effective
absenceSupport(X) :- not_supports(Y, X), node(Y), node(X).
hasSupport(X) :- node(X), relation(Y, X, 1), not absenceSupport(X).
% acceptable is true if it has support AND is not attacked by an acceptable argument.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{argumentation_stratified_scatter.pdf}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{argumentation_stratified_time.pdf}
...
\begin{table}[ht]
    ...
    \begin{tabular}{lllllll}
        \toprule
        \# Nodes & \multicolumn{2}{c}{\textsc{C2D}} & \multicolumn{2}{c}{\textsc{D4}} & \multicolumn{2}{c}{\textsc{SharpSAT-TD}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        & Size & Time (s) & Size & Time (s) & Size & Time (s)\\
        \midrule
        1 & 107 & 0.001 & \textbf{99} & \textbf{0.00014} & 160 & 0.001225 \\
        2 & 2555 & 0.002 & 2736 & \textbf{0.00105} & \textbf{1637} & 0.003164 \\
        3 & 170844 & 0.044 & 123025 & 0.04816 & \textbf{88729} & \textbf{0.016818} \\
        4 & 101566925 & 37.9 & 6183424 & 3.96765 & \textbf{2057170} & \textbf{0.264196} \\
        5 & - & - & - & - & \textbf{21386450} & \textbf{2.93743} \\
        6 & - & - & - & - & \textbf{436225040} & \textbf{65.4520} \\
        \bottomrule
    \end{tabular}
    \caption{Comparison of top-down compilers over Listing~\ref{lst:ungrounded_pasp_2}.
    The table compares circuit size (number of nodes plus edges) and compilation time
    (in seconds) for \textsc{C2D}, \textsc{D4}, and \textsc{SharpSAT-TD}. The best
    results for each row are highlighted in bold. A dash (-) indicates that the
    compiler was unable to complete the compilation within a reasonable time frame
    (30 minutes) or ran out of memory.}
