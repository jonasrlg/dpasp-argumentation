...
\chapter{Appendix Answer Set Programming}
\label{ap:asp}

The study of \acl{KR} aspires to find compromises between
...
\ac{KB} can be intractable \citep{levesque1986knowledge}. Even
...
Due to this intrinsic complexity of \ac{FOL}, the main 
alternative to \ac{KB} representation is the use of less 
...
fundamental trade-off that characterizes the \ac{KR} research
\citep{levesque1987expressiveness}.

In the previous appendix \ref{ap:sat}, we talked about the
complexity class of \ac{SAT}-like encompasses several hard
...
first-order logical \ac{KB} \citep{brachman2004knowledge}. In 
other words, Resolution is an inference procedure that leads to
a refutation-complete theorem-proving technique for 
Propositional Logic and \ac{FOL}.

As one could imagine, by its relationship with \ac{SAT}, in its 
...
ourselves to only a certain interesting subset of \ac{FOL}, 
...
subset in question, \ac{ASP} is still sufficiently expressive 
...
Before diving on the semantics of \ac{PASP}, it is important to
...
the basis where \ac{ASP} is built upon, but also hint at the 
underlying complexity of the problems that \ac{ASP} can solve
and the efficiency of the algorithms that can be used in this 
context. Thus, we start by defining the simplest form of 
\acl{LP}, called \textit{definite} (or \textit{positive}) 
programs:
...
\begin{definition}[Definite Logic Programs]
    ...
    \acl{FOL} $L$; and this rule can be seen as material 
...
\begin{definition}[Interpretation]
   ...
   I$. Although, \acl{Normal} can have multiple minimal models 
...
propositional theories are fed into \ac{SAT} solvers to compute
...
that include negation, we introduce the concept of \acl{ASP},
...
We start this \ac{ASP} definition by first introducing the 
concept of \ac{ELP} and \ac{EDLP}, programs that use the three
...
\ac{ELP}, because one can emulate this concept by using 
...
\begin{definition}[Extended Logic Programs]
    ...
    \textit{strongly negated} atoms of a \acl{FOL}.
\end{definition}
...
models of \acp{ELP} are called \textbf{\textit{answer sets}}.
...
\begin{definition}[Extended Disjunctive Logic Programs]
    ...
    \textit{strongly negated} atoms of a \acl{FOL}; and the 
    symbol $;$ denotes disjunction (similar to how commas
    denote conjunctions).
\end{definition}

The semantics for and \ac{EDLP} can be defined similarly to the
ones for an \ac{ELP} or normal logic program, with two main 
...
Finally, we define Intepretations for \acp{EDLP}, that unifies
different types of extensions that lead from 
\citep{wielemaker2012swi} to the development of \acl{ASP}:

\begin{definition}[Interpretation (of \acp{EDLP}]
    Let $P$ be an \acl{EDLP}. An interpretation $I$ is a model
    of:
    \begin{enumerate}
...
The last extension that characterizes \ac{ASP} is called
\textit{cardinality constraint}, which is are constructs of the 
...
\begin{example}
    ...
    \begin{minted}{prolog}
        {a} :- b, not c.
    \end{minted}
    is equivalent to the \acl{Normal} program $P'$:
    \begin{minted}{prolog}
        a :- not aa, b, not c.
        aa:- not a.
    \end{minted}
